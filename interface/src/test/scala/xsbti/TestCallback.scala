...
class TestCallback(override val nameHashing: Boolean = false) extends AnalysisCallback
{
	...
	val binaryDependencies = new ArrayBuffer[(URL, String, File, DependencyContext)]
	...
	def binaryDependency(binary: URL, name: String, source: File, inherited: Boolean): Unit = {
		val context = if(inherited) DependencyByInheritance else DependencyByMemberRef
		binaryDependency(binary, name, source, context)
	}
	def binaryDependency(binary: URL, name: String, source: File, context: DependencyContext): Unit = { binaryDependencies += ((binary, name, source, context)) }
	def generatedClass(source: File, module: URL, name: String): Unit = { products += ((source, module, name)) }
