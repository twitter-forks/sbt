...
trait Relations {
  ...
  def allBinaryDeps: collection.Set[ClassRef]
  ...
  def addProduct(src: File, prod: ClassRef, name: String): Relations
  ...
  def addBinaryDep(src: File, dependsOn: ClassRef): Relations
  ...
  def addSource(src: File,
    products: Iterable[(ClassRef, String)],
    internalDeps: Iterable[InternalDependency],
    externalDeps: Iterable[ExternalDependency],
    binaryDeps: Iterable[(ClassRef, String, Stamp)]): Relations =
   ...
   */
  private[inc] def addProducts(src: File, prods: Iterable[(ClassRef, String)]): Relations
   ...
   */
  private[inc] def addBinaryDeps(src: File, deps: Iterable[(ClassRef, String, Stamp)]): Relations
  ...
  def srcProd: Relation[File, ClassRef]

  /** The dependency relation between internal sources and binaries. */
  def binaryDep: Relation[File, ClassRef]
...
object Relations {
  ...
  def construct(nameHashing: Boolean, relations: List[Relation[_, _]]) =
    relations match {
      case p :: bin :: di :: de :: pii :: pie :: mri :: mre :: ii :: ie :: cn :: un :: Nil =>
        val srcProd = p.asInstanceOf[Relation[File, ClassRef]]
        val binaryDep = bin.asInstanceOf[Relation[File, ClassRef]]
  ...
  private[this] lazy val e = Relation.empty[File, ClassRef]
  ...
  def make(srcProd: Relation[File, ClassRef], binaryDep: Relation[File, ClassRef], direct: Source, publicInherited: Source, classes: Relation[File, String]): Relations =
    new MRelationsDefaultImpl(srcProd, binaryDep, direct = direct, publicInherited = publicInherited, classes)

  private[inc] def make(srcProd: Relation[File, ClassRef], binaryDep: Relation[File, ClassRef],
 ...
 */
private abstract class MRelationsCommon(val srcProd: Relation[File, ClassRef], val binaryDep: Relation[File, ClassRef],
    val classes: Relation[File, String]) extends Relations {
  def allSources: collection.Set[File] = srcProd._1s

  def allProducts: collection.Set[ClassRef] = srcProd._2s
  def allBinaryDeps: collection.Set[ClassRef] = binaryDep._2s
  ...
  def products(src: File): Set[ClassRef] = srcProd.forward(src)
  def produced(prod: ClassRef): Set[File] = srcProd.reverse(prod)

  def binaryDeps(src: File): Set[ClassRef] = binaryDep.forward(src)
  def usesBinary(dep: ClassRef): Set[File] = binaryDep.reverse(dep)
 ...
 *
 */
private class MRelationsDefaultImpl(srcProd: Relation[File, ClassRef], binaryDep: Relation[File, ClassRef],
  ...
  def addProducts(src: File, products: Iterable[(ClassRef, String)]): Relations =
  ...
  def addBinaryDeps(src: File, deps: Iterable[(ClassRef, String, Stamp)]) =
  ...
  def groupBy[K](f: File => K): Map[K, Relations] =
    {
      ...
      def outerJoin(srcProdMap: MapRel[ClassRef], binaryDepMap: MapRel[ClassRef], direct: Map[K, Source],
 ...
 */
private class MRelationsNameHashing(srcProd: Relation[File, ClassRef], binaryDep: Relation[File, ClassRef],
