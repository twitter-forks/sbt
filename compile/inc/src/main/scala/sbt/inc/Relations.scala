...
trait Relations {
  ...
  def allBinaryDeps: collection.Set[FileRef]
  ...
  def addProduct(src: File, prod: FileRef, name: String): Relations
  ...
  def addBinaryDep(src: File, dependsOn: FileRef): Relations
  ...
  def addSource(src: File,
    products: Iterable[(FileRef, String)],
    internalDeps: Iterable[InternalDependency],
    externalDeps: Iterable[ExternalDependency],
    binaryDeps: Iterable[(FileRef, String, Stamp)]): Relations =
   ...
   */
  private[inc] def addProducts(src: File, prods: Iterable[(FileRef, String)]): Relations
   ...
   */
  private[inc] def addBinaryDeps(src: File, deps: Iterable[(FileRef, String, Stamp)]): Relations
  ...
  def srcProd: Relation[File, FileRef]

  /** The dependency relation between internal sources and binaries. */
  def binaryDep: Relation[File, FileRef]
...
object Relations {
  ...
  private[inc] val existingRelations = {
    ...
    val string2Ref: String => FileRef = FileRefs.fromString(_)
    List(
      ("products", string2Ref),
  ...
  def construct(nameHashing: Boolean, relations: List[Relation[_, _]]) =
    relations match {
      case p :: bin :: di :: de :: pii :: pie :: mri :: mre :: ii :: ie :: cn :: un :: Nil =>
        val srcProd = p.asInstanceOf[Relation[File, FileRef]]
        val binaryDep = bin.asInstanceOf[Relation[File, FileRef]]
  ...
  private[this] lazy val e = Relation.empty[File, FileRef]
  private[this] lazy val ef = Relation.empty[File, File]
  private[this] lazy val estr = Relation.empty[File, String]
  private[this] lazy val es = new Source(ef, estr)

  def emptySource: Source = es
  private[inc] lazy val emptySourceDependencies: SourceDependencies = new SourceDependencies(ef, estr)
  ...
  def make(srcProd: Relation[File, FileRef], binaryDep: Relation[File, FileRef], direct: Source, publicInherited: Source, classes: Relation[File, String]): Relations =
    new MRelationsDefaultImpl(srcProd, binaryDep, direct = direct, publicInherited = publicInherited, classes)

  private[inc] def make(srcProd: Relation[File, FileRef], binaryDep: Relation[File, FileRef],
 ...
 */
private abstract class MRelationsCommon(val srcProd: Relation[File, FileRef], val binaryDep: Relation[File, FileRef],
    val classes: Relation[File, String]) extends Relations {
  def allSources: collection.Set[File] = srcProd._1s

  def allProducts: collection.Set[FileRef] = srcProd._2s
  def allBinaryDeps: collection.Set[FileRef] = binaryDep._2s
  ...
  def products(src: File): Set[FileRef] = srcProd.forward(src)
  def produced(prod: FileRef): Set[File] = srcProd.reverse(prod)

  def binaryDeps(src: File): Set[FileRef] = binaryDep.forward(src)
  def usesBinary(dep: FileRef): Set[File] = binaryDep.reverse(dep)
 ...
 *
 */
private class MRelationsDefaultImpl(srcProd: Relation[File, FileRef], binaryDep: Relation[File, FileRef],
  ...
  def addProducts(src: File, products: Iterable[(FileRef, String)]): Relations =
  ...
  def addBinaryDeps(src: File, deps: Iterable[(FileRef, String, Stamp)]) =
  ...
  def groupBy[K](f: File => K): Map[K, Relations] =
    {
      ...
      def outerJoin(srcProdMap: MapRel[FileRef], binaryDepMap: MapRel[FileRef], direct: Map[K, Source],
 ...
 */
private class MRelationsNameHashing(srcProd: Relation[File, FileRef], binaryDep: Relation[File, FileRef],
