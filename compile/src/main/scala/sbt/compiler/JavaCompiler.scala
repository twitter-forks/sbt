...
trait JavaCompiler extends xsbti.compile.JavaCompiler {
   ...
   * @param outputLocation The output directory for class files
   * @param options The arguments to pass into Javac
   * @param log  A log in which we write all the output from Javac.
   */
  def apply(sources: Seq[File], classpath: Seq[File], outputLocation: File, options: Seq[String])(implicit log: Logger): Unit
